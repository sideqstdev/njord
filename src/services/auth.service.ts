...
import {
  authTokenInterface,
  authUserInterface,
} from "../types/interfaces/authuser.interface";
...
export const regenerateRefreshToken = (userShard: authUserInterface) => {
  try {
    ...
    throw new Error(
      `Error whilst regenerating a refresh token: ${dev ? err : null}`
    );
  }
};
...
export const decodeTokenExp = async (
  token: string,
  type?: tokenTypes
): Promise<Date> => {
  const secret = type === `refresh` ? refreshSecret : accessSecret;
